...
def main():
    ...
    # get the W matrix
    In = np.eye(s_lbp.shape[0])
    W = (1/4) * ((s_lbp - In)/(np.sum(s_lbp) - s_lbp.shape[0]) + (s_hog - In)/(np.sum(s_hog)\
    ...
    pca_dc = dc.PCA(n_components=64) 
    trainned_base = pca_dc.fit_transform(S.transpose())
    ...
    for idx in tqdm(range(test_length)):
        ...
        s_low_dim = pca_dc.transform(s_test.transpose())
    ...
    with open('./results/pca_traditional.pkl', 'wb') as f:
